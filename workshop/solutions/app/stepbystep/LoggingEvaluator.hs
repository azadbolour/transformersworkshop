...
runTransformerStack :: Env -> OperationCount -> TransformerStack Identity val -> ((Either String val, [String]), OperationCount)
runTransformerStack env = \initialCount effectfulVal -> runIdentity(runStateT(runWriterT(runExceptT (runReaderT effectfulVal env))) initialCount)
